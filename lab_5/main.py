            ...
            match args.enc_or_dec:
                case 'encryption':
                    match args.key:
                        ...
                        case 'generate':
                            while (not args.key_size.isdigit() or int(args.key_size) % 8 != 0 or
                                   int(args.key_size) < 2048):
                                args.key_size = input('Ключ RSA введен неверно.\n'
                                                      'Попробуйте ввести ещё раз: ')
                            RSA.generate_key(int(args.key_size))
                            file_operation.get_pub_and_priv_key_to_file(RSA, settings['rsa_public_key'],
